...
# 時の歌
# D Sneak S D Sneak S
scoreboard players set #pattern_id Flag 0
...
execute as @e[type=armor_stand,tag=OcarinaPattern,tag=ScoreTarget] run scoreboard players set @s Flag 3
...
# パターンに一致した場合の処理
execute if entity @s[tag=Found] as @a[tag=UsingFlute] run function ocarina:music/start/toki
